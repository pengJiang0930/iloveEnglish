...
| 词书管理 | ✅ | 完整 CRUD + 上下架 |
| 单词管理 | ✅ | 按词书筛选 + 分页 + CRUD |
| AI速记管理 | 🔲 | 待开发（依赖 word_memory_tip 模型） |
| 用户管理 | ✅ | 列表 + 筛选 + 分页 + 启用/禁用 |
| 样式优化 | ✅ | 修复页面留白 + 按钮间距问题 |
...
| 端点 | 状态 | 说明 |
|------|------|------|
| `GET /api/admin/dashboard` | ✅ | 首页统计数据（词书/单词/用户数） |
| `POST/PUT/DELETE /api/word-books` | ✅ | 词书 CRUD + 上下架 |
| `POST/PUT/DELETE /api/words/admin` | ✅ | 单词 CRUD |
